...
class ProductController extends Controller
{
    private function resolveProductOrFail(int|string $id): Product
    {
        ...
        $query = Product::query()->withoutGlobalScope('tenant')->whereKey($id);
    ...
    public function store(Request $request)
    {
        ...
        $showAsBanner = (bool) ($validated['show_as_banner'] ?? false);
        if ($showAsBanner && !$isPackage) {
            return back()->withErrors(['show_as_banner' => 'Banner hanya untuk produk paket.'])->withInput();
        }
    ...
    public function destroy($product)
    {
        ...
        try {
            ...
            if (DB::table('package_items')->where('item_product_id', $product->id)->exists()) {
                return back()->withErrors(['product' => 'Produk tidak dapat dihapus karena digunakan dalam paket produk lain.']);
            }

            // Check if product is used in any reservation spaces
            if (DB::table('reservation_space_items')->where('product_id', $product->id)->exists()) {
                return back()->withErrors(['product' => 'Produk tidak dapat dihapus karena digunakan dalam ruang reservasi.']);
            }
