...
@section('header')
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">Ubah Grup Add-on · {{ $product->name }}</h2>
@endsection
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.products.options.update', [$product, $option]) }}" class="grid gap-4" x-data="optionFormPreview({
                        name: @js(old('name', $option->name)),
                        type: @js(old('type', $option->type)),
                        isRequired: @js((bool) old('is_required', $option->is_required)),
                        isActive: @js((bool) old('is_active', $option->is_active)),
                    })">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <input name="name" x-model="name" value="{{ old('name', $option->name) }}" class="mt-1 w-full rounded-xl border-gray-300" required />
                        ...
                        <div>
                            ...
                            <select name="type" x-model="type" class="mt-1 w-full rounded-xl border-gray-300" required>
                                <option value="single" {{ old('type', $option->type)==='single' ? 'selected' : '' }}>Pilih satu (radio)</option>
                                <option value="multi" {{ old('type', $option->type)==='multi' ? 'selected' : '' }}>Bisa pilih banyak (checkbox)</option>
                            </select>
                            @error('type')<div class="text-sm text-red-600 mt-1">{{ $message }}</div>@enderror
                        </div>

                        <label class="inline-flex items-center gap-2">
                            <input type="checkbox" name="is_required" value="1" x-model="isRequired" {{ old('is_required', $option->is_required) ? 'checked' : '' }} class="rounded border-gray-300" />
                            <span class="text-sm">Wajib dipilih</span>
                        </label>

                        <label class="inline-flex items-center gap-2">
                            <input type="checkbox" name="is_active" value="1" x-model="isActive" {{ old('is_active', $option->is_active) ? 'checked' : '' }} class="rounded border-gray-300" />
                        ...
                        <div>
                            ...
                            <input name="sort_order" type="number" min="0" max="10000" value="{{ old('sort_order', $option->sort_order) }}" class="mt-1 w-full rounded-xl border-gray-300" />
