...
<body class="min-h-screen text-[#1A1A1A]" style="background: var(--background-color); font-family: var(--font-family)">
    <div class="mx-auto max-w-6xl px-4">
        ...
        @if($showHeader)
            <header class="sticky top-0 z-[120] py-4 bg-[#1A1A1A]/90 backdrop-blur supports-[backdrop-filter]:bg-[#1A1A1A]/80">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        @endif
                        <div>
                            <div class="text-lg font-semibold leading-tight" style="color: #FFFFFF">{{ $brand?->business_name ?? 'Self Order' }}</div>
                            <div class="text-xs" style="color: #F5C518">{{ $brand?->tagline ?? 'Cafe & Resto' }}</div>
                        </div>
                    </div>
                    <div class="flex items-center justify-end gap-3">
                        @if($headerRightText !== '')
                            <div class="text-right text-xs text-[#888888]">
                                {!! $headerRightHtml !!}
                            </div>
                        @endif
                                ...
                                @php
                                    $profileUrl = \Illuminate\Support\Facades\Route::has('profile.edit')
                                        ...
                                        : (\Illuminate\Support\Facades\Route::has('tenant.profile.edit') && request()->route('tenant')
                                            ...
                                            : null);
                                @endphp
                                <button type="button" @click="open = !open" class="flex items-center gap-2 rounded-2xl border border-[#2D2D2D] bg-[#1A1A1A] px-3 py-2 text-xs font-semibold text-white" style="border-radius: var(--button-radius)">
                                    <span class="h-7 w-7 rounded-xl grid place-items-center text-[#1A1A1A] font-black" style="background: #F5C518">
                                        {{ strtoupper(substr(auth()->user()->name ?? 'M', 0, 1)) }}
                                    </span>
                                    <span>Member</span>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="h-4 w-4 text-[#888888]">
    ...
    <div class="fixed inset-x-0 bottom-0 z-50">
        <div class="mx-auto max-w-6xl px-4 pb-[calc(env(safe-area-inset-bottom)+12px)] pt-2">
            <nav x-data="{ active: '{{ $defaultActive }}' }"
                class="grid grid-cols-4 gap-1 rounded-3xl border border-[#2D2D2D] bg-[#1A1A1A] backdrop-blur shadow-lg p-2"
                x-init="
                    // Update navigation links with table token from localStorage if available
                    try {
                        const tableToken = localStorage.getItem('tableToken');
                        if (tableToken) {
                            const links = $el.querySelectorAll('a[href*=\'order.page\'], a[href*=\'customer/orders\']');
                            links.forEach(link => {
                                try {
                                    const url = new URL(link.href, window.location.origin);
                                    if (!url.searchParams.has('table')) {
                                        url.searchParams.set('table', tableToken);
                ...
                <a href="{{ $homeUrl }}" @click="active='home'"
                    ...
                    :class="active==='home' ? 'text-[#F5C518]' : 'text-[#888888] hover:bg-[#2D2D2D]'"
                    :style="active==='home' ? 'background: #1A1A1A; border-radius: var(--button-radius)' : 'border-radius: var(--button-radius)'">
                ...
                <a href="{{ $orderPageUrl }}#menu" @click="active='menu'"
                    class="flex flex-col items-center justify-center gap-1 rounded-2xl px-2 py-2 text-[11px] font-semibold"
                    :class="active==='menu' ? 'text-[#F5C518]' : 'text-[#888888] hover:bg-[#2D2D2D]'"
                    :style="active==='menu' ? 'background: #1A1A1A; border-radius: var(--button-radius)' : 'border-radius: var(--button-radius)'">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="h-6 w-6">
                ...
                <a href="{{ $pesananPageUrl }}" @click="active='pesanan'"
                    class="flex flex-col items-center justify-center gap-1 rounded-2xl px-2 py-2 text-[11px] font-semibold"
                    :class="active==='pesanan' ? 'text-[#F5C518]' : 'text-[#888888] hover:bg-[#2D2D2D]'"
                    :style="active==='pesanan' ? 'background: #1A1A1A; border-radius: var(--button-radius)' : 'border-radius: var(--button-radius)'">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="h-6 w-6">
                ...
                <a href="{{ $promoUrl }}" @click="active='promo'"
                    class="flex flex-col items-center justify-center gap-1 rounded-2xl px-2 py-2 text-[11px] font-semibold"
                    :class="active==='promo' ? 'text-[#F5C518]' : 'text-[#888888] hover:bg-[#2D2D2D]'"
                    :style="active==='promo' ? 'background: #1A1A1A; border-radius: var(--button-radius)' : 'border-radius: var(--button-radius)'">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="h-6 w-6">
