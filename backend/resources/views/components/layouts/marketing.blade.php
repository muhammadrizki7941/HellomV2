...
<body class="antialiased text-gray-900 bg-white">
    ...
    <main>
        <div class="container mx-auto px-6 lg:px-20 py-10">
            @if (session('success'))
                <div class="bg-green-50 border border-green-200 text-green-800 rounded p-3 mb-4">{{ session('success') }}</div>
            @endif
            @if (session('error'))
                <div class="bg-red-50 border border-red-200 text-red-800 rounded p-3 mb-4">{{ session('error') }}</div>
            @endif

            {{ $slot }}

            <div class="text-sm text-gray-500 mt-8">Foundation mode: no DB / no migrations.</div>
        </div>
    </main>

    <footer class="bg-white">
        <div class="container mx-auto px-6 lg:px-20 py-8 text-center text-sm text-gray-500">© {{ date('Y') }} SaaS Kasir</div>
    </footer>
</body>
</html>
