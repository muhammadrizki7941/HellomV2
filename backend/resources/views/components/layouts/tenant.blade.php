...
<head>
    ...
    <style>
        ...
        .btn{display:inline-block; background:#2563eb; color:white; padding:10px 14px; border-radius:12px; font-weight:700; border:0;}
        .muted{color:#94a3b8;}
    </style>
</head>
<body>
    <div class="wrap">
        ...
        @if (session('success'))
            <div class="card" style="border-color:#14532d; background:#052e16; margin-bottom:12px;">{{ session('success') }}</div>
        @endif
        @if (session('error'))
            <div class="card" style="border-color:#7f1d1d; background:#450a0a; margin-bottom:12px;">{{ session('error') }}</div>
        @endif
